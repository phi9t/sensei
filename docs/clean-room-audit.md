...
| Backward cost         | Rendered block and timeline geometry preserve the V1 `F = 1`, `B = 2` duration model; labels and metrics reported durations 1 and 2.                                                   |
...
| Desktop workbench     | The current UI targets block-first play: blocks and the schedule board lead, move inspector stays contextual, and controls, metrics, and timeline details are progressively disclosed. |
...
| Offline reload        | With an activated worker and cache `sensei-shell-b46addd54eea8763`, the preview server was stopped and the page reloaded with the correct title, heading, and activated worker.        |
...
- current worktree status after the release gate should be reviewed with `git status --short`; generated `dist/` and TypeScript build-info files are ignored.
